...
export class Shape {
  ...
  constructor({
    ...
    counterText.fontSize = this.counterTextfontSize;
    ...
    personImg.position = new Point(
      -personImg.bounds.width - 3,
      -personImg.bounds.width * 0.25
    );
  ...
  get pointPos() {
    ...
    delta = delta.add(
      new Point(delta.x > 0 ? dx : -dx, delta.y > 0 ? dy : -dy)
    );
  ...
  applyForces(dt) {
    ...
    let k = this.springConstant,
      ...
      ay =
        -k * (this.pos.y - view.size.height / 2) + Iy * this.invmass - a * vy;
  ...
  interactionImpulse(other: Shape, i: number, j: number) {
    let tol = 2,
      ...
      maxDistance =
        this.radius * (1 - this.intersections[j]) +
        other.radius * (1 - other.intersections[i]) +
        tol;
...
let personSvg: SVGElement = document.createElementNS(
  "http://www.w3.org/2000/svg",
  "svg"
);
personSvg.innerHTML =
  '<defs><style>.cls-1{fill:#052B47;}</style></defs><g><path class="cls-1" d="M81.05,53.1H71a29.65,29.65,0,1,0-36.17,0H25.37c-14,0-25.37,8-25.37,38.05V101a81.66,81.66,0,0,0,106.46.29c0-4.09-.07-7.84,0-10.18C106.79,63.14,95.06,53.1,81.05,53.1Z"/></g>';
